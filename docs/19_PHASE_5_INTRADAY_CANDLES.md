...
- 第一筆有效 update 建立 OHLC；sequence duplicate、舊 sequence、舊 timestamp 不更新 close。
- cumulative volume/turnover 以 Redis baseline 計算 delta。Snapshot 只更新 baseline；reset 產生零 delta、STALE 診斷，不使用負值或 `abs()`。
- 下一 bucket 到達時 finalize 前一根。無成交分鐘不補假 candle。Session close finalization 由既有 calendar/session lifecycle 觸發邊界負責。
...
- Retention 集中設定為最近 5 個交易日目標（key TTL 7 日，涵蓋週末）；不寫 PostgreSQL 或 Room。
...
- 訂閱 candle channel 先收到 `candle_snapshot`，後續收到 `candle`；payload 含 `interval` 與 `is_final`。
- `GET /v1/intraday/{market}/{code}/candles` 支援 `interval`, `date`, `from`, `to`, `limit`，不掃描 Redis keys。

## Android 1D Chart

- Security Detail 的 `1D` 使用 intraday REST snapshot + WebSocket updates，1m/5m 可切換。
