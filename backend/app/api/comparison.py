...
async def run_comparison(
    ...
    sec_schemas = [
        ComparisonSecuritySummarySchema(
            ...
            name=s.name,
            market=s.market,
            latest_close=str(s.latest_close) if s.latest_close is not None else None,
            ...
            return_10d=str(s.return_10d) if s.return_10d is not None else None,
            return_20d=str(s.return_20d) if s.return_20d is not None else None,
            return_60d=str(s.return_60d) if s.return_60d is not None else None,
            return_selected_window=str(s.return_selected_window) if s.return_selected_window is not None else None,
            ...
            close_vs_ma20=str(s.close_vs_ma20) if s.close_vs_ma20 is not None else None,
            close_vs_ma60=str(s.close_vs_ma60) if s.close_vs_ma60 is not None else None,
            rsi14=str(s.rsi14) if s.rsi14 is not None else None,
            macd_state=s.macd_state,
            kd_state=s.kd_state,
            foreign_1d_net=str(s.foreign_1d_net) if s.foreign_1d_net is not None else None,
            foreign_5d_net=str(s.foreign_5d_net) if s.foreign_5d_net is not None else None,
            trust_1d_net=str(s.trust_1d_net) if s.trust_1d_net is not None else None,
            trust_5d_net=str(s.trust_5d_net) if s.trust_5d_net is not None else None,
            dealer_1d_net=str(s.dealer_1d_net) if s.dealer_1d_net is not None else None,
            dealer_5d_net=str(s.dealer_5d_net) if s.dealer_5d_net is not None else None,
            margin_balance_change=str(s.margin_balance_change) if s.margin_balance_change is not None else None,
            short_balance_change=str(s.short_balance_change) if s.short_balance_change is not None else None,
            lending_balance_change=str(s.lending_balance_change) if s.lending_balance_change is not None else None,
            industry_name=s.industry_name,
            themes=s.themes,
            industry_strength_score=str(s.industry_strength_score) if s.industry_strength_score is not None else None,
