    ...
    private fun parseQuoteMap(m: Map<*, *>): RealtimeQuote? {
        return try {
            RealtimeQuote(
                ...
                bidPrices = (m["bid_prices"] as? List<*>)?.mapNotNull { it?.toString() } ?: emptyList(),
                bidVolumes = (m["bid_volumes"] as? List<*>)?.mapNotNull { (it as? Number)?.toInt() } ?: emptyList(),
                askPrices = (m["ask_prices"] as? List<*>)?.mapNotNull { it?.toString() } ?: emptyList(),
                askVolumes = (m["ask_volumes"] as? List<*>)?.mapNotNull { (it as? Number)?.toInt() } ?: emptyList(),
