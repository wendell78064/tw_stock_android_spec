...
import tw.market.ledger.model.AlertEvaluationMode
...
@Composable fun CreateAlertRoute(prefillSecurity: String? = null, prefillPrice: String? = null,
    ...
    val state by vm.state.collectAsState()
    val status = state.dashboardOrNull()?.realtimeProviderStatus ?: "UNCONFIGURED"
    CreateAlertScreen(prefillSecurity, prefillPrice, initialType, status) { vm.save(it); onDone() }
}
...
@Composable
fun CreateAlertScreen(prefillSecurity: String? = null, prefillPrice: String? = null, initialType: AlertType = AlertType.PRICE_TARGET, realtimeProviderStatus:String="UNCONFIGURED", onSave: (AlertRuleInput) -> Unit = {}) {
    var type by remember { mutableStateOf(initialType) }; var scope by remember { mutableStateOf(AlertScope.SECURITY) }
    var mode by remember { mutableStateOf(AlertEvaluationMode.EOD) }
    ...
    Column(Modifier.padding(12.dp)) {
        ...
        Row(Modifier.testTag("alert-evaluation-mode")) { AlertEvaluationMode.entries.forEach { value -> TextButton({ mode = value }) { Text(if(value==AlertEvaluationMode.EOD) "盤後" else "盤中即時") } } }
        if(mode==AlertEvaluationMode.REALTIME && realtimeProviderStatus=="UNCONFIGURED") Text("即時行情尚未配置，規則目前不會觸發",Modifier.testTag("realtime-provider-unavailable"))
        ...
        val supported = if(mode==AlertEvaluationMode.EOD) AlertType.entries else AlertType.entries.filter { it !in setOf(AlertType.MA_CLOSE_ABOVE,AlertType.MA_CLOSE_BELOW,AlertType.MA_CONSECUTIVE_ABOVE,AlertType.MA_CONSECUTIVE_BELOW) }
        supported.forEach { value -> TextButton({ type = value }) { Text(value.name) } }
...
fun NotificationCenterScreen(state: AlertUiState, read: (AlertEvent) -> Unit = {}, readAll: () -> Unit = {}) {
    ...
    Column(Modifier.padding(12.dp)) { Text("通知中心"); Text("未讀 ${data!!.events.count { it.readAt == null }}"); TextButton(readAll) { Text("全部已讀") }
        LazyColumn { items(data.events) { event -> Card(Modifier.fillMaxWidth().padding(4.dp)) { Column(Modifier.padding(8.dp)) {
            val readLabel = if (event.readAt == null) "未讀" else "已讀"; val modeLabel=if(event.evaluationMode==AlertEvaluationMode.REALTIME) "盤中" else "盤後";Text("$readLabel · $modeLabel · ${event.securityCode} ${event.securityName}"); Text(event.message)
            Text("${if(event.evaluationMode==AlertEvaluationMode.REALTIME) "成交價" else "收盤"} ${event.triggerPrice} · ${event.referenceType} ${event.referenceValue}"); Text("資料日 ${event.tradeDate} · ${event.dataStatus}"); TextButton({ read(event) }) { Text("標記已讀") }
        } } } }
    }
}
