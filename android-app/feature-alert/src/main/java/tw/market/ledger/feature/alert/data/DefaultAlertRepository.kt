...
class DefaultAlertRepository @Inject constructor(private val api:AlertApi,private val dao:AlertDao,private val realtime:RealtimeQuoteClient):AlertRepository {
 override suspend fun dashboard()=try { realtime.subscribeChannels("alert");val rules=api.rules().data; val events=api.notifications().data;val status=api.realtimeStatus().data; dao.clearRules();dao.upsertRules(rules.map(::entity));dao.upsertEvents(events.map(::entity));AlertDashboard(rules.map(::model),events.map(::model),realtimeProviderStatus=status.providerStatus) } catch(_:IOException){ AlertDashboard(dao.rules().map(::model),dao.events().map(::model),true) }
...
private fun model(x:AlertRuleDto)=AlertRule(x.id,x.name,AlertType.valueOf(x.ruleType),AlertScope.valueOf(x.scopeType),x.enabled,x.maPeriod,x.thresholdPrice,x.thresholdPercent,x.consecutiveDays,x.cooldownMinutes,x.dailyLimit,AlertEvaluationMode.valueOf(x.evaluationMode))
...
private fun model(x:AlertEventDto)=AlertEvent(x.id,x.securityCode,x.securityName,x.triggeredAt,x.tradeDate,x.eventType,x.triggerPrice,x.referenceValue,x.referenceType,x.message,x.dataStatus,x.readAt,AlertEvaluationMode.valueOf(x.evaluationMode),x.fingerprint)
